const text = `99lbqpxzzlbtvkmfrvrnmcxttseven
q7cnfslbtpkvseven
...
function findCalibrationValue(line: string) {
    ...
    const firstLastDigit = findLastDigit(line)

    return firstDigit + firstLastDigit;
}
...
function findFirstDigit(line: string) {
    ...
    for (let digit of digits.keys()) {
        const index = line.indexOf(digit)
        if (index !== -1 && (index < firstDigit.index || firstDigit.index === -1)) {
            firstDigit = {
...
function findLastDigit(line: string) {
    ...
    for (let digit of digits.keys()) {
        ...
        if (index !== -1 && index > lastDigit.index) {
            lastDigit = {
                value: digits.get(digit),
                index: index
            }
