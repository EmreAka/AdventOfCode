const text = `.......497...........................858...923...128..................227..801........487.....664...........................................
...
for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    const lineToWork = lines[lineIndex]
    const lineAfter = lines[lineIndex]

    const lineNumbers = getLineNumbers(lineBefore, lineToWork, lineAfter)
    const lineSymbols = getLineSymbols(lineBefore, lineToWork, lineAfter)
    const line: Line = {
        lineIndex: lineIndex,
        numbers: lineNumbers,
        symbols: lineSymbols
...
for (let i = 0; i < lineObjects.length; i++) {
    let lineBefore = lineObjects[i - 1]
    let line = {...lineObjects[i]};
    let lineAfter = lineObjects[i + 1]

    let lineSum = 0;

    // Line Before
    if (lineBefore && lineBefore.symbols.length > 0 && line.numbers.length > 0) {
        for (let number of line.numbers) {
            for (let symbol of lineBefore.symbols) {
                if (symbol.index >= number.firstIndex - 1 && symbol.index <= number.lastIndex + 1 && !number.visited) {
                    //sum += +number
                    number.visited = true
                    lineSum += +number.number;
                }
            }
        }
    }
    ...
    // Line Actual
    if (line.symbols.length > 0 && line.numbers.length > 0) {
        for (let number of line.numbers) {
            for (let symbol of line.symbols) {
                if (symbol.index >= number.firstIndex - 1 && symbol.index <= number.lastIndex + 1 && !number.visited) {
                    //sum += +number
                    number.visited = true
                    lineSum += +number.number;
    ...
    // Line After
    if (lineAfter && lineAfter.symbols.length > 0 && line.numbers.length > 0) {
        for (let number of line.numbers) {
            for (let symbol of lineAfter.symbols) {
                if (symbol.index >= number.firstIndex - 1 && symbol.index <= number.lastIndex + 1 && !number.visited) {
                    //sum += +number
                    number.visited = true
                    lineSum += +number.number;
    ...
    console.log(`Line: ${line.lineIndex} - Line Sum: ${lineSum}`)
    sum += lineSum
    lineSum = 0
}
...
// Helper Functions
function getLineSymbols(lineBefore?: string, lineToWork?: string, lineAfter?: string): LineSymbol[] {
    const lineSymbols: LineSymbol[] = [];

    for (let charIndex = 0; charIndex < lineToWork.length; charIndex++) {
        let character = lineToWork[charIndex]

        let digit = parseInt(character)

        if (isNaN(digit) && character !== '.') {
            lineSymbols.push({
                index: charIndex,
                symbol: character
            })
...
function getLineNumbers(lineBefore?: string, lineToWork?: string, lineAfter?: string): LineNumber[] {
    let lineNumbers: LineNumber[] = [];
    let number = "";
    let startingIndex = 0;

    for (let charIndex = 0; charIndex < lineToWork.length; charIndex++) {
        let char = parseInt(lineToWork[charIndex])
        if (!isNaN(char) && number.length === 0) {
            number += char
            ...
            if (number.length > 0)
                lineNumbers.push({
                    ...
                    lastIndex: startingIndex + number.length - 1,
                    number: number,
                    visited: false
                })
    ...
    if(number.length > 0 && lineNumbers.length === 0) {
        lineNumbers.push({
            firstIndex: startingIndex,
            lastIndex: startingIndex + number.length - 1,
            number: number,
