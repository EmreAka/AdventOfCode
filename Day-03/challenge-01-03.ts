const textInput = `.......497...........................858...923...128..................227..801........487.....664...........................................
...
type CellType = 'digit' | 'separator' | 'symbol';

solution()

function solution() {
    let lines = textInput.split("\n")
    let myHeight = lines.length
    let myWidth = lines[0].length

    let myGigaChadGrid: GridCell[][] = []
    let symbols: GridCell[] = []
    let numbers: MyNumber[] = []

    for (let i = 0; i < myHeight; i++) {
        let gridArray: GridCell[] = []
        for (let j = 0; j < myWidth; j++) {
            let myGigaChadCell: GridCell = {
                y: i,
                x: j,
                myChar: lines[i][j],
                cellType: getCellType(lines[i][j])
            }

            gridArray.push(myGigaChadCell)
        }
        myGigaChadGrid.push(gridArray)
    }

    for (let cellList of myGigaChadGrid) {
        for (let cell of cellList) {
            if (cell.cellType === 'symbol') symbols.push(cell)
        }
    }

    for (let cellList of myGigaChadGrid) {
        const myNumbers = getNumbers(cellList);
        numbers.push(...myNumbers)
    }

    for (let number of numbers) {
        const symbolsAtTheSameLevel = symbols.filter(symbol => symbol.y === number.y)
        const symbolsAtBottomLevel = symbols.filter(symbol => symbol.y === (number.y + 1))
        const symbolsAtTopLevel = symbols.filter(symbol => symbol.y === (number.y - 1))

        markAdjacentTheNumber(number, symbolsAtTheSameLevel)
        markAdjacentTheNumberForSymbols(number, symbolsAtBottomLevel)
        markAdjacentTheNumberForSymbols(number, symbolsAtTopLevel)
    }
    ...
    let sum = 0;
    for (let number of numbers) {
        if(number.adjacent) sum += number.value
    }
    console.log(sum)
}

function markAdjacentTheNumber(number: MyNumber, symbols: GridCell[]) {
    const xCoordinatesToLookFor = [(number.x[0] - 1), number.x[number.x.length - 1] + 1]
    if (symbols.length > 0) {
        for (let symbol of symbols) {
            for (let coordinate of xCoordinatesToLookFor) {
                if (coordinate === symbol.x) number.adjacent = true
            }
        }
    }
}
function markAdjacentTheNumberForSymbols(number: MyNumber, symbols: GridCell[]) {
    const xCoordinatesToLookFor = [(number.x[0] - 1), ...number.x ,number.x[number.x.length - 1] + 1]
    if (symbols.length > 0) {
        for (let symbol of symbols) {
            for (let coordinate of xCoordinatesToLookFor) {
                if (coordinate === symbol.x) number.adjacent = true
...
function getNumbers(cellList: GridCell[]): MyNumber[] {
    let number = ""
    let numberIndexes: number[] = []
    let numbers: MyNumber[] = [];

    for (let cell of cellList) {
        if (cell.cellType === 'digit') {
            number += cell.myChar
            numberIndexes.push(cell.x)
        } else {
            if (number.length > 0)
                numbers.push({
                    x: numberIndexes,
...
function getCellType(char: string): CellType {
    ...
    if (isNaN(parseInt(char)) && char === '.') return 'separator'
    else return 'symbol'
}
