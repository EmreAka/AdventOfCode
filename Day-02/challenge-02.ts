...
function run(games: Game[]) {
    ...
    for (let game of games) {
        ...
        for (let record of game.records) {
            if (record.blue > gameRecord.blue) gameRecord.blue = record.blue
            if (record.green > gameRecord.green) gameRecord.green = record.green
            if (record.red > gameRecord.red) gameRecord.red = record.red
        }

        sum += (gameRecord.blue * gameRecord.green * gameRecord.red)
    }
