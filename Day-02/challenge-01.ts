const text = `Game 1: 12 red, 2 green, 5 blue; 9 red, 6 green, 4 blue; 10 red, 2 green, 5 blue; 8 blue, 9 red
...
const possibleGameRecord: GameRecord = {
    red: 12,
    green: 13,
    blue: 14
}
...
parse(games)
const result = run(gameObjects)
console.log(result)


// Helper Functions
function run(games: Game[]) {
    let sum = 0;

    for (let game of games) {
        let totalGameRecord: GameRecord = {
            blue: 0,
            red: 0,
            green: 0
        }
        for (let record of game.records) {
            if (record.red > totalGameRecord.red) totalGameRecord.red = record.red
            if (record.blue > totalGameRecord.blue) totalGameRecord.blue = record.blue
            if (record.green > totalGameRecord.green) totalGameRecord.green = record.green
        }

        if (totalGameRecord.red <= possibleGameRecord.red
            && totalGameRecord.blue <= possibleGameRecord.blue
            && totalGameRecord.green <= possibleGameRecord.green
        ) {
            sum += game.id;
        }
    }
...
function parse(games: string[]) {
    for (let game of games) {
        const gameId = getGameId(game)
        const gameObject: Game = {
            id: gameId,
            records: getRecords(game)
        }
...
function getGameId(game: string) {
    const gameString = game.split(":")[0]
    const gameId = gameString.split(' ')[1]
    return +gameId
}

function getRecords(game: string): GameRecord[] {
    const recordsString = game.split(":")[1]
    const records = recordsString.split(';')
    ...
    for (let record of records) {
        ...
        const colors: ParsedColor[] = trimmedRecord.split(',').map(color => {
            const colorString = color.trim().split(' ')
            return {
                piece: +colorString[0],
                color: colorString[1] as Color
            }
        })

        let gameRecord: GameRecord = {};
        for (let color of colors) {
            gameRecord[color.color] = color.piece
        }
